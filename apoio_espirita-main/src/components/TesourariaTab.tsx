...
import {
  ...
  Trash2, ChevronLeft, ChevronRight, Calendar, Download, Printer, X
} from "lucide-react";
    ...
    <section className="space-y-6 animate-fade-in-up" style={{ animationDuration: '400ms' }}>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className={`rounded-2xl p-4 border ${saldo >= 0 ? "bg-cyan-50 border-cyan-100" : "bg-rose-50 border-rose-100"}`}>
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <p className="text-[10px] uppercase tracking-wider text-gray-400 font-bold select-none">Saldo</p>
          </div>
          <p className={`text-sm sm:text-base font-semibold ${saldo >= 0 ? "text-cyan-700" : "text-rose-600"}`}>
            {fmtBRL(saldo)}
          </p>
        </div>
        <div className="rounded-2xl p-4 bg-emerald-50 border border-emerald-100">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <p className="text-[10px] uppercase tracking-wider text-gray-400 font-bold select-none">Receitas</p>
          </div>
        ...
        <div className="rounded-2xl p-4 bg-rose-50 border border-rose-100">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <p className="text-[10px] uppercase tracking-wider text-gray-400 font-bold select-none">Despesas</p>
          </div>
      ...
      {canTesouraria && showForm && (
        <div className="glass rounded-3xl p-6 mb-6 space-y-4 border border-white/20">
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* Valor */}
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-1">Valor (R$) *</label>
              <input
                type="text"
                inputMode="decimal"
            ...
            {/* Data */}
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-1">Data *</label>
              <input
                type="date"
                value={fData}
      ...
      {loadingTx ? (
        ...
        <div className="space-y-2.5">
          {transacoes.map((tx) => (
            <div key={tx.id} className="bg-white rounded-2xl border border-gray-100 shadow-xs px-4 py-3.5 flex items-center gap-3">
              {/* Indicador de tipo */}
              <div className={`shrink-0 w-1 h-8 rounded-full ${tx.tipo === "receita" ? "bg-emerald-400" : "bg-rose-400"}`} />
              ...
              <div className="flex-1 min-w-0">
                ...
                {tx.observacao && (
                  <p className="text-[10px] text-gray-300 mt-0.5 truncate font-light italic">{tx.observacao}</p>
                )}
              </div>
