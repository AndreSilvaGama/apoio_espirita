...
const BACKGROUNDS = [
  ...
  {
    id: "bg-gradient-to-br from-indigo-50 to-cyan-50",
    label: "Gradiente Celestial",
    css: "bg-gradient-to-br from-indigo-50 to-cyan-50 text-gray-800",
  },
  {
    id: "bg-gradient-to-br from-blue-50 to-purple-50",
    label: "Gradiente Harmonia",
    css: "bg-gradient-to-br from-blue-50 to-purple-50 text-gray-800",
  },
  {
    id: "bg-gradient-to-br from-emerald-50 to-teal-50",
    label: "Gradiente Vitalidade",
    css: "bg-gradient-to-br from-emerald-50 to-teal-50 text-gray-800",
  },
  {
    id: "bg-gradient-to-br from-amber-50 to-rose-50",
    label: "Gradiente Fraternidade",
    css: "bg-gradient-to-br from-amber-50 to-rose-50 text-gray-800",
  },
];
...
function getPrazoInfo(prazo: string | null, concluido: boolean): { cor: string; texto: string } {
  ...
  if (concluido)
    return {
      cor: "text-emerald-600 bg-emerald-50 border-emerald-200 px-2 py-0.5 rounded",
      texto: "Concluído",
    };
  ...
  if (data < hoje)
    return {
      cor: "text-red-600 bg-red-50 border-red-200 px-2 py-0.5 rounded font-medium",
      texto: "Atrasado",
    };
  if (data <= amanha)
    return {
      cor: "text-amber-600 bg-amber-50 border-amber-200 px-2 py-0.5 rounded font-medium",
      texto: "Amanhã",
    };
...
/**
 * Carga que o dnd-kit transporta em cada elemento arrastavel/soltavel.
 * Os valores sao definidos em KanbanColumnWrapper e KanbanCardWrapper.
 */
type DragData = { type?: "card" | "list" | "column"; listId?: string } | undefined;
...
function KanbanPage() {
  ...
  const [config, setConfig] = useState<{ board_background: string; share_token: string } | null>(
    null,
  );
  ...
  const fetchBoardData = async (siglaCasa: string) => {
    ...
    try {
      ...
      const stored =
        typeof window !== "undefined" ? localStorage.getItem(`kanban_board_${siglaCasa}`) : null;
      const chosenBoardId =
      ...
      if (
        ...
        !guestToken &&
        user &&
        !(frentesData || []).some((f) => f.board_id === chosenBoardId)
      ) {
      ...
      const storedFrente =
        typeof window !== "undefined"
          ? localStorage.getItem(`kanban_frente_${chosenBoardId}`)
          : null;
      const boardFrentes = frentesList
        .filter((f) => f.board_id === chosenBoardId)
      ...
      if (
        (!listasData || listasData.length === 0) &&
        chosenBoardId &&
        chosenFrenteId &&
        !guestToken &&
        user
      ...
      setEventos((cardsData as unknown as KanbanEvento[]) || []);
      ...
      // profiles_public devolve colunas anulaveis: descarta linhas sem id/nome
      setMembrosCasa(
        (membersData ?? [])
          .filter((m): m is { id: string; nome: string } => !!m.id && !!m.nome)
  ...
  const fetchCardDetails = async (cardId: string) => {
    ...
    try {
      ...
      setGruposCard(sortedGrupos as unknown as KanbanGrupo[]);
  ...
  const switchBoard = (boardId: string) => {
    ...
    const storedFrente =
      typeof window !== "undefined" ? localStorage.getItem(`kanban_frente_${boardId}`) : null;
    const fId =
      (storedFrente && bf.some((f) => f.id === storedFrente) && storedFrente) ||
      (bf[0]?.id ?? null);
  ...
  const handleAddBoard = async () => {
    ...
    try {
      ...
      setShowBoardMenu(false);
      if (typeof window !== "undefined" && sigla)
        localStorage.setItem(`kanban_board_${sigla}`, data.id);
  ...
  const handleRenameBoard = async (boardId: string) => {
    ...
    try {
      ...
      if (error) throw error;
      setBoards(
        boards.map((b) => (b.id === boardId ? { ...b, nome: editingBoardName.trim() } : b)),
      );
  ...
  const handleDeleteBoard = async (boardId: string, boardName: string) => {
    ...
    if (!confirm(`Excluir o projeto "${boardName}"? Todas as listas e cards dele serão excluídos!`))
      return;
    const client = getClient();
    try {
      const { error } = await client.from("kanban_boards").delete().eq("id", boardId);
    ...
    try {
      const { data, error } = await client
        ...
        .insert({
          ...
          nome: newFrenteName.trim(),
          ordem: boardFrentes.length,
          membros: [],
        })
        .select()
        .single();
  ...
  const handleRenameFrente = async (frenteId: string) => {
    ...
    try {
      ...
      if (error) throw error;
      setFrentes(
        frentes.map((f) => (f.id === frenteId ? { ...f, nome: editingFrenteName.trim() } : f)),
      );
  ...
  const handleDeleteFrente = async (frenteId: string, nome: string) => {
    ...
    if (!confirm(`Excluir a frente "${nome}"? Todas as listas e cards dela serão excluídos!`))
      return;
    const client = getClient();
    try {
  ...
  const handleDeleteList = async (listId: string, listName: string) => {
    if (!confirm(`Excluir a lista "${listName}"? Todos os cards nela também serão excluídos!`))
      return;
    const client = getClient();
    try {
      const { error } = await client.from("kanban_listas").delete().eq("id", listId);
  ...
  const handleCreateCard = async (listId: string) => {
    ...
    try {
      ...
      if (error) throw error;
      setEventos([...eventos, data as unknown as KanbanEvento]);
      setNewCardTitle("");
  ...
  const handleDeleteCard = async (cardId: string) => {
    ...
    const client = getClient();
    try {
      const { error } = await client.from("kanban_eventos").delete().eq("id", cardId);
  ...
  const findListIdByCard = (cardId: string) =>
    eventos.find((e) => e.id === cardId)?.lista_id || null;

  const handleDragStart = (event: DragStartEvent) => {
    const data = event.active.data.current as DragData;
    // "column" e apenas area de solta, nunca origem de arrasto
    if (data?.type === "card" || data?.type === "list") {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    else if (overData?.type === "column") targetListId = overData.listId ?? null;
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeData?.type === "list") {
      ...
      const ordered = listas
        .filter((l) => l.frente_id === activeFrenteId)
        .sort((a, b) => a.ordem - b.ordem);
      ...
      try {
        await Promise.all(
          reordered.map((l) =>
            client.from("kanban_listas").update({ ordem: l.ordem }).eq("id", l.id),
          ),
        );
      } catch (err) {
    ...
    if (activeData?.type === "card") {
      ...
      let working = [...eventos];
      const listCards = working
        .filter((e) => e.lista_id === activeListId)
        .sort((a, b) => a.ordem - b.ordem);
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    const client = getClient();
    try {
      const { error } = await client.from("kanban_comentarios").delete().eq("id", commentId);
  ...
  const handleAddChecklist = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setGruposCard([...gruposCard, { ...data, kanban_tarefas: [] } as unknown as KanbanGrupo]);
      setNewChecklistName("");
  ...
  const handleDeleteChecklist = async (grupoId: string, name: string) => {
    ...
    const client = getClient();
    try {
      const { error } = await client.from("kanban_grupos").delete().eq("id", grupoId);
  ...
  const handleAddChecklistItem = async (grupoId: string) => {
    ...
    try {
      ...
      setGruposCard(
        gruposCard.map((g) => {
          if (g.id === grupoId) {
            return { ...g, kanban_tarefas: [...g.kanban_tarefas, data] } as KanbanGrupo;
          }
          return g;
        }),
      );
  ...
  const handleToggleChecklistItem = async (tarefaId: string, feito: boolean, grupoId: string) => {
    const client = getClient();
    try {
      const { error } = await client.from("kanban_tarefas").update({ feito }).eq("id", tarefaId);
      ...
      setGruposCard(
        gruposCard.map((g) => {
          if (g.id === grupoId) {
            return {
              ...g,
              kanban_tarefas: g.kanban_tarefas.map((t) =>
                t.id === tarefaId ? { ...t, feito } : t,
              ),
            };
          }
          return g;
  ...
  const handleDeleteChecklistItem = async (tarefaId: string, grupoId: string) => {
    const client = getClient();
    try {
      const { error } = await client.from("kanban_tarefas").delete().eq("id", tarefaId);
  ...
  const filteredEventos = eventos.filter((evento) => {
    ...
    // 4. Member Filter
    if (filterMember) {
      if (!evento.membros_atribuidos || !evento.membros_atribuidos.includes(filterMember))
        return false;
    }
    ...
    if (filterPrazo) {
      ...
      if (
        filterPrazo === "amanha" &&
        (data.getTime() !== amanha.getTime() || evento.prazo_concluido)
      )
        return false;
  ...
  const boardFrentes = frentes
    .filter((f) => f.board_id === activeBoardId)
    .sort((a, b) => a.ordem - b.ordem);
  const activeFrente = boardFrentes.find((f) => f.id === activeFrenteId) || null;
  const frenteListas = listas
    .filter((l) => l.frente_id === activeFrenteId)
    .sort((a, b) => a.ordem - b.ordem);
  const activeDragCard =
    activeDrag?.type === "card" ? eventos.find((e) => e.id === activeDrag.id) : null;
  const activeDragList =
    activeDrag?.type === "list" ? frenteListas.find((l) => l.id === activeDrag.id) : null;
  ...
  return (
    <main className={`min-h-screen ${activeBg.id} pt-20 pb-20 transition-all duration-500`}>
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {guestToken && (
          <div className="mb-4 bg-amber-500/10 border border-amber-500/20 text-amber-800 rounded-xl px-4 py-2 flex items-center justify-between text-xs">
            <p className="font-light">
              <span className="font-semibold">Quadro Compartilhado:</span> Você está acessando este
              quadro de projetos via link de convite.
            </p>
          </div>
        )}
        ...
        <div className="glass rounded-2xl p-4 md:p-6 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border border-white/20">
          <div>
            <div className="flex items-center gap-3">
              <Link
                to={user && sigla ? "/casa/$sigla" : "/inicio"}
                params={user && sigla ? { sigla } : undefined}
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span
                    ...
                    className="truncate max-w-[60vw] group-hover:text-cyan-700 transition-colors"
                  >
                          ...
                          <div className="p-1 space-y-1.5">
                            ...
                            <div className="flex gap-1.5">
                              <button
                                ...
                                className="flex-1 py-1.5 text-xs font-semibold bg-[#004a8c] text-white rounded-lg hover:bg-[#003c73] cursor-pointer"
                              >
                                Criar projeto
                              </button>
                              <button
                                ...
                                className="p-1.5 border border-gray-200 text-gray-500 rounded-lg hover:bg-gray-50 cursor-pointer"
                              >
                                <X size={13} />
                              </button>
                            </div>
              ...
              {boardFrentes.map((f) => {
                ...
                if (editingFrenteId === f.id) {
                  return (
                    <div key={f.id} className="flex items-center gap-1 px-2 py-1.5">
                      ...
                      />
                      <button
                        onClick={() => handleRenameFrente(f.id)}
                        className="p-1 bg-cyan-600 text-white rounded hover:bg-cyan-700 cursor-pointer"
                      >
                        <Check size={12} />
                      </button>
                      <button
                        onClick={() => setEditingFrenteId(null)}
                        className="p-1 border border-gray-200 text-gray-500 rounded hover:bg-gray-50 cursor-pointer"
                      >
                        <X size={12} />
                      </button>
                ...
                return (
                  <button
                    ...
                    onDoubleClick={() => {
                      if (user) {
                        setEditingFrenteId(f.id);
                        setEditingFrenteName(f.nome);
                      }
                    }}
                  ...
                  >
                    ...
                    {f.nome}
                    {f.membros.length > 0 && (
                      <span className="text-[10px] text-gray-400">· {f.membros.length}</span>
                    )}
                    {user && ativa && (
                      <span className="flex items-center gap-0.5 ml-1">
                        <span
                          ...
                          className="p-0.5 rounded hover:bg-gray-200/60 text-gray-400"
                        >
                          <Pencil size={11} />
                        </span>
                        <span
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDeleteFrente(f.id, f.nome);
                          }}
                ...
                (showNewFrenteForm ? (
                  <div className="flex items-center gap-1 px-2 py-1.5">
                    ...
                    <button
                      ...
                      className="p-1 bg-[#004a8c] text-white rounded hover:bg-[#003c73] cursor-pointer"
                    >
                      <Check size={12} />
                    </button>
                    <button
                  ...
                  <button
                    onClick={() => setShowNewFrenteForm(true)}
                    className="flex items-center gap-1 px-3 py-2.5 text-xs font-semibold text-gray-500 hover:text-cyan-700 whitespace-nowrap cursor-pointer"
                  >
            ...
            {activeFrente && (
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                ...
                {activeFrente.membros.length === 0 ? (
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    {activeFrente.membros.map((nome) => (
                      <span
                        key={nome}
                        className="flex items-center gap-1 text-[11px] bg-white border border-gray-200 rounded-full pl-1 pr-2 py-0.5 text-gray-700"
                      >
                        <span
                          className={`w-4 h-4 rounded-full ${avatarColor(nome)} text-white text-[7px] font-bold flex items-center justify-center`}
                        >
                          {avatarInitials(nome)}
                        </span>
                        {nome}
                ...
                {user && (
                  <div className="relative">
                    <button
                      onClick={() => setShowFrenteMembros((v) => !v)}
                      className="text-[11px] font-semibold text-cyan-600 hover:text-cyan-700 cursor-pointer flex items-center gap-1"
                    >
                    ...
                    {showFrenteMembros && (
                      <div className="absolute left-0 top-full mt-1 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-20 p-2 max-h-60 overflow-y-auto">
                        <div className="flex items-center justify-between px-1 pb-1 mb-1 border-b border-gray-100">
                          ...
                          <button
                            onClick={() => setShowFrenteMembros(false)}
                            className="text-gray-400 hover:text-gray-600"
                          >
                            <X size={12} />
                          </button>
                        ...
                        {membrosCasa.length === 0 ? (
                          ...
                          membrosCasa.map((m) => {
                            ...
                            return (
                              <button
                                key={m.id}
                                onClick={() => toggleFrenteMembro(activeFrente.id, m.nome)}
                                className="w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs text-left hover:bg-gray-50 cursor-pointer text-gray-700"
                              >
                                <span className="flex items-center gap-1.5">
                                  <span
                                    className={`w-4 h-4 rounded-full ${avatarColor(m.nome)} text-white text-[7px] font-bold flex items-center justify-center`}
                                  >
                                    {avatarInitials(m.nome)}
                                  </span>
                                  {m.nome}
                                </span>
        ...
        {/* Board lists layout */}
        {loadingBoard ? (
          <div className="py-20 text-center text-sm text-gray-500 font-light">
            Carregando quadro de projetos...
          </div>
        ) : (
          <DndContext
            sensors={sensors}
              ...
              >
                {frenteListas.map((lista) => (
                  <KanbanColumnWrapper
                    key={lista.id}
                    list={lista}
                    cards={filteredEventos
                      .filter((e) => e.lista_id === lista.id)
                      .sort((a, b) => a.ordem - b.ordem)}
            ...
            >
              {activeDragCard ? (
                ...
                <div className="min-w-[272px] max-w-[272px] rounded-2xl border border-cyan-300 bg-white shadow-xl p-4 rotate-2 cursor-grabbing">
                  <h3 className="text-xs font-bold text-gray-700 uppercase tracking-wider truncate">
                    {activeDragList.nome}
                  </h3>
                  <p className="text-[10px] text-gray-400 mt-1">
                    {eventos.filter((e) => e.lista_id === activeDragList.id && !e.arquivado).length}{" "}
                    card(s)
                  </p>
                </div>
              ) : null}
              ...
              <div className="flex-1 pr-4">
                ...
                <p className="text-[10px] text-gray-400 mt-1 font-light flex items-center gap-1">
                  ...
                  <strong className="font-semibold">
                    {listas.find((l) => l.id === selectedCard.lista_id)?.nome}
                  </strong>
            ...
            <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedCard.labels &&
                    selectedCard.labels.map((lId) => {
                      const tag = ETIQUETAS.find((t) => t.id === lId);
                      if (!tag) return null;
                      return (
                        <span
                          key={lId}
                          className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${tag.bg}`}
                        >
                          {tag.label}
                        </span>
                      );
                    })}
                  {selectedCard.membros_atribuidos &&
                    selectedCard.membros_atribuidos.map((mNome) => (
                      <span
                        key={mNome}
                        className="text-[10px] font-medium bg-cyan-50 border border-cyan-100 text-cyan-700 px-2 py-0.5 rounded-full flex items-center gap-1"
                      >
                        <User size={10} />
                        {mNome}
                      </span>
                  ...
                  {selectedCard.data && (
                    <span
                      className={`text-[10px] font-medium border flex items-center gap-1 ${getPrazoInfo(selectedCard.data, selectedCard.prazo_concluido).cor}`}
                    >
                      <Clock size={10} />
                      {fmtData(selectedCard.data)}{" "}
                      {getPrazoInfo(selectedCard.data, selectedCard.prazo_concluido).texto}
                    </span>
                  )}
                </div>
                  ...
                  {editingDescription ? (
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            handleUpdateCard({
                              ...selectedCard,
                              descricao: tempDescription.trim() || null,
                            });
                            setEditingDescription(false);
                          }}
                    ...
                    <p className="text-xs text-gray-600 font-light whitespace-pre-wrap bg-gray-50/50 border border-gray-100 rounded-xl p-3 leading-relaxed">
                      {selectedCard.descricao || (
                        <span className="italic text-gray-400">Nenhuma descrição adicionada.</span>
                      )}
                    </p>
                  )}
                </div>
                  ...
                  {selectedCard.anexos && selectedCard.anexos.length > 0 && (
                    <div className="space-y-1.5 max-w-md">
                      {selectedCard.anexos.map((anexo, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl px-3 py-2 text-xs"
                        >
                          <a
                            href={anexo.url}
                            target="_blank"
                ...
                <div className="space-y-4">
                  ...
                  {loadingCardDetails ? (
                    ...
                    <p className="text-[10px] text-gray-400 italic">
                      Nenhum checklist criado para este projeto.
                    </p>
                  ) : (
                    <div className="space-y-4">
                      {gruposCard.map((grupo) => {
                        ...
                        return (
                          <div
                            key={grupo.id}
                            className="bg-white border border-gray-150 rounded-xl p-4 shadow-xs space-y-3"
                          >
                            <div className="flex items-center justify-between">
                            ...
                            {grupo.kanban_tarefas.length > 0 && (
                              <div className="space-y-2 border-t border-gray-50 pt-2">
                                {grupo.kanban_tarefas.map((tarefa) => (
                                  <div
                                    key={tarefa.id}
                                    className="flex items-start justify-between gap-2 group/task text-xs"
                                  >
                                    <label className="flex items-start gap-2 flex-1 cursor-pointer">
                                      ...
                                      />
                                      <span
                                        className={`${tarefa.feito ? "line-through text-gray-400" : "text-gray-600"}`}
                                      >
                                        {tarefa.titulo}
                                      </span>
                                    </label>
                    ...
                    <div className="space-y-3 pt-2 max-w-xl">
                      {comentariosCard.map((comment) => {
                        ...
                        return (
                          <div
                            key={comment.id}
                            className="flex items-start gap-3 bg-gray-50/45 border border-gray-100 rounded-2xl p-3 animate-fade-in-up"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-gray-600 font-light mt-1 whitespace-pre-wrap">
                                {comment.comentario}
                              </p>
                            </div>
                          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  >
                    <span className="flex items-center gap-1.5">
                      <Tag size={13} className="text-gray-400" /> Etiquetas
                    </span>
                ...
                <div className="relative">
                  ...
                  >
                    <span className="flex items-center gap-1.5">
                      <Users size={13} className="text-gray-400" /> Membros
                    </span>
                    <ChevronDown size={12} className="text-gray-400" />
                  </button>
                  ...
                  {showAddMemberMenu && (
                    <div className="absolute left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl p-2 shadow-lg z-10 max-h-48 overflow-y-auto space-y-0.5">
                      {membrosCasa.map((m) => {
                        const active =
                          selectedCard.membros_atribuidos &&
                          selectedCard.membros_atribuidos.includes(m.nome);
                        return (
                          <button
                            key={m.id}
                            onClick={() => {
                    ...
                    {COVERS.map((c) => (
                      <button
                        ...
                        onClick={() =>
                          handleUpdateCard({
                            ...selectedCard,
                            cover: selectedCard.cover === c.id ? null : c.id,
                          })
                        }
                        title={c.id}
                ...
                <div className="bg-gray-50 border border-gray-150 rounded-xl p-4 space-y-3">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleUpdateCard({ ...selectedCard, data: e.target.value || null })
                      }
                  ...
                  {selectedCard.data && (
                    <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-gray-600">
                      <input
                        ...
                        onChange={(e) =>
                          handleUpdateCard({ ...selectedCard, prazo_concluido: e.target.checked })
                        }
                ...
                <div className="relative">
                  ...
                  >
                    <span className="flex items-center gap-1.5">
                      <ChevronRight size={13} className="text-gray-400" /> Mover para Lista
                    </span>
                    <ChevronDown size={12} className="text-gray-400" />
                  </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {filteredEventos.length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-8 italic">
                  Nenhum card arquivado nesta busca.
                </p>
              ) : (
  ...
  return (
    ...
    >
      ...
      <div ref={setCardDropRef} className="space-y-3 pr-1 min-h-[12px]">
        ...
        {cards.length === 0 && (
          <div className="text-[10px] text-gray-300 italic text-center py-3 select-none">
            Solte cards aqui
          </div>
        )}
      </div>
...
function CardPresentation({
  ...
  const assigned =
    card.membros_atribuidos && card.membros_atribuidos.length > 0
          ...
          <div className="flex flex-wrap gap-1 mb-2 pr-6">
            {card.labels.map((lId) => {
              ...
              return (
                <span
                  key={lId}
                  className={`w-3.5 h-1.5 rounded-full ${tag.dot}`}
                  title={tag.label}
                ></span>
              );
            })}
        ...
        {(card.data || assigned.length > 0 || (card.anexos && card.anexos.length > 0)) && (
          <div className="mt-2.5 pt-2 border-t border-gray-50 flex items-center justify-between flex-wrap gap-1.5 text-[9px] text-gray-400 font-light">
            <div className="flex items-center gap-1.5">
              {card.data && (
                <span
                  className={`flex items-center gap-0.5 ${getPrazoInfo(card.data, card.prazo_concluido).cor.split(" ")[0]}`}
                >
                  {card.prazo_concluido ? (
